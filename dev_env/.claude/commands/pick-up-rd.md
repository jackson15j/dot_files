---
description: Fetch, assign a Linear issue, branch, implement and push a PR (rd / rvu-default)
...
> **rd variant**: identical to pick-up-v2 but runs on `model: rvu-default`
> (behind the LiteLLM proxy) for model-selection testing. Its internal review
> step calls `/cr-rd` (not `/cr-v2`) so the whole flow stays on the proxy model.
>
> **v2 changes vs v1** (see Phase notes): risk-based main-loop/sub-agent split
> instead of phase-based; an adversarial self-review BEFORE first push; a
...
## Phase 0 — Detect the repo's toolchain (you do this directly)
...
## Phase 1 - Linear + branch setup (dispatch to an `rvu-default` subagent)
...
## Phase 2 — Plan + scope (you do this directly)
...
- **Keep on the main loop, edit directly:** the semantically deep / high-risk
  pieces — contract shapes, custom-decoder or middleware interactions, nil-able
  types, test helpers that inject defaults, anything where one context needs to
  hold the whole picture. These are exactly the files that cause repeat review
  rounds when reviewed via summary.
- **Delegate to an `rvu-default` sub-agent** (Agent tool, `subagent_type:
  general-purpose`, `model: rvu-default`): mechanical breadth — regenerating
...
## Phase 3.5 — Adversarial self-review BEFORE first push (you do this directly)

This is the new gate that prevents pushing red / leaving findings for the bot.

1. Invoke `/cr-rd` against `origin/main` and clear every finding it raises —
...
2. Fix the CLASS, not the cited line (see /cr-rd Step 4).
...
Dispatch an Agent (general-purpose, model: rvu-default) with prompt:
...
## Phase 5 — Ship (you do this directly)
...
4. When review comments arrive, address them via `/cr-rd` (fix-the-class +
